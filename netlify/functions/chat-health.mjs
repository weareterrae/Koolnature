// Ping horário ao motor de IA do Chef Kool. Se cair, avisa por email — e avisa
...
export default async () => {
  ...
  try { store = getStore("chef-kool-health"); antes = (await store.get("estado")) || "up"; } catch { /* sem blobs: avisa na mesma na 1ª deteção */ }

  if (estado !== antes) {
    const assunto = up ? "✅ O Chef Kool recuperou — koolnature.pt" : "🔴 O Chef Kool em baixo — motor de IA (KoolNature)";
    const texto = up
      ? "O motor de IA do Chef Kool voltou a responder ao ping horário. Tudo normal."
      : "O motor de IA do Chef Kool (Gemini, o principal) NÃO respondeu ao ping horário. O site tem o Claude como rede de segurança, mas convém verificar a GEMINI_API_KEY no Netlify e a conta Google AI Studio.";
    try {
